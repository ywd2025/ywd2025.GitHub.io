      ...
      >
        <div class="title_part">
          <div>
            ...
            <h1 class="now_title">{{ $t("new.records.transfer") }}</h1>
          </div>
        </div>
        <div class="table_all">
          ...
          <div class="table_part">
            <Table :columns="columns" :data="data" :no-data-text="$t('common.nodata')">
              <template slot-scope="{ row, index }" slot="tranSource">
                {{ formatter(row.tranSource) }}
              </template>
              <template slot-scope="{ row, index }" slot="tranTarget">
                {{ formatter(row.tranTarget) }}
              </template>
            </Table>
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          title: this.$t('carouselItem.text161'),
          key: "amount",
          render: (h, params) => {
            return h(
              "span",
        ...
        {
          title: this.$t('carouselItem.text162'),
          slot: "tranSource",
        },
        {
          title: this.$t('carouselItem.text163'),
          slot: "tranTarget",
        },
      ],
      data: [],
  ...
  computed: {
    formatter() {
      return (status) => {
        switch (status) {
         case 0:
            return this.$t('carouselItem.text147');
          case 1:
            return this.$t('carouselItem.text148');
          case 2:
            return this.$t('carouselItem.text149');
          case 9:
            return this.$t('carouselItem.text150');
          default:
            break;
        }
      };
