<template>
  <div class="notice">
    <Row :gutter="30">
      <Col :xs="24" :sm="24" :md="5" :lg="5" class="notice-list">
        <div class="main">
          ...
          <div class="list">
            <div
              :class="item.id == queryId ? 'active' : 'item1'"
              v-for="item in FAQList"
              @click="noticedeail(item.id)"
            >
          ...
          <div v-show="hasContent">
            ...
            <div class="nav-bottom">
              <div class="left" v-if="data.back">
                <router-link
                  class="link"
                  :to="'../announcement/' + data.back.id"
                >
                  < {{ $t("cms.prevnotice") }}
                  <p style="color:#03C59E;">{{ data.back.title }}</p>
                </router-link>
              </div>
              <div class="right" v-if="data.next">
                <router-link
                  class="link"
                  :to="'../announcement/' + data.next.id"
                  >{{ $t("cms.nextnotice") }} >
                  <p style="color:#03C59E;">{{ data.next.title }}</p>
                </router-link>
              </div>
            </div>
          </div>
          <div v-show="!hasContent">
            <p style="font-size: 30px;text-align:center;margin-top: 15%;">
              <Icon type="ios-cafe" />
            </p>
            <p
              style="text-align:center;font-size: 12px;margin-top: 10px;color: #828ea1;"
            >
              {{ $t("cms.notexist") }}
            </p>
          </div>
    ...
    <div class="bottom-list">
      <p style="font-size: 18px;margin: 15px 0;">{{$t('carouselItem.text70')}}</p>
      <div
        class="notice-item"
        v-for="item in FAQList"
        @click="noticedeail(item.id)"
      >
        <span class="text"
          >[{{ item.createTime | subTime }}] {{ item.title }}</span
        >
      </div>
    </div>
    <!-- <div class="app_bottom">
      <div class="left_logo">
        <img style="float:left;" :src="$imgUrl + '/index/applogo.png'" />
...
export default {
  ...
  methods: {
    ...
    loadNoticeInfo() {
      ...
      this.qrcode.value = this.host + "/#/announcement/" + id;
      this.queryId = id;
