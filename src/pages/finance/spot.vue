        ...
        <Tabs value="加密貨幣">
          <TabPane :label="$t('new.universal.assets')" name="加密貨幣">
            <div class="otherCoins_content">
              <div class="filte_all">
                <Checkbox v-model="single" @on-change="handleCheckChange()">{{$t('carouselItem.text206')}}</Checkbox>
                <Input
                type="text"
                v-model="searchValue"
              ...
              <Table :columns="encryptionColumns" :data="encryptionData" :no-data-text="$t('common.nodata')">
  ...
  watch: {
    searchValue(newVal, oldVal) {
      ...
      this.encryptionData = this.allData.data.filter((item) => {
        return item.coinName.toString().toLowerCase().includes(newVal);
      });
    },
  },
  methods: {
    ...
    handleCheckChange(){
      if(this.single == true){
         this.encryptionData = this.allData.data.filter((item) => Number(item.useUSDT) != 0)
      }else{
