...
export default {
  ...
  methods: {
    initCanvas() {
      ...
      this.context_cover.lineCap = 'round'
      this.initHoverEvent()
    },
    initPoints() {
      ...
      if (buy != undefined && sell != undefined) {
        ...
        this.yObj.min = 0
        let bidValue = null
        this.values.bid.items.length >= 1 && (bidValue = this.values.bid.items[this.values.bid.items.length - 1].amount)
        this.values.bid.items.length == 0 && (bidValue = 0)
        let askValue = null
        this.values.ask.items.length >= 1 && (askValue = this.values.ask.items[this.values.ask.items.length - 1].amount)
        ...
        var yScale = parseFloat((this.pHeight - 50) / (this.yObj.max - this.yObj.min)).toFixed(4)
        this.buyPoints = []
        for (var i = buyArr.length - 1; i >= 0; i--) {
          var x = (buyArr.length - 1 - i) * xScale
          var y = Math.floor(this.pHeight - 50 - (buyArr[i].amount - this.yObj.min) * yScale)
        ...
        for (var i = 0; i < sellArr.length; i++) {
          ...
          var y = Math.floor(this.pHeight - 50 - (sellArr[i].amount - this.yObj.min) * yScale)
    ...
    drawSell() {
      ...
      this.context.lineTo(this.pWidth - 5, this.sellPoints[this.sellPoints.length - 1].y)
      this.context.lineTo(this.pWidth - 5, this.pHeight - 0)
      this.context.fillStyle = '#392231'
      if (this.values.skin === 'day') {
        this.context.fillStyle = '#FDE7E6'
      } else {
        this.context.fillStyle = '#392231'
      }
      this.context.fill()
    ...
    drawAxis() {
      ...
      for (var i = 0; i < this.buyPoints.length; i++) {
        if (i % 10) {
          ...
          this.context.fillStyle = '#000000'
          this.context.font = '12px Adobe Ming Std'
          this.context.fillText(this.buyPoints[i].price, this.buyPoints[i].x - 10, this.pHeight - 30)
        }
      }
      ...
      for (var i = 0; i < this.sellPoints.length; i++) {
        if (i % 10) {
          ...
          this.context.fillStyle = '#000000'
          this.context.font = '12px Adobe Ming Std'
          this.context.fillText(this.sellPoints[i].price, this.sellPoints[i].x - 10, this.pHeight - 30)
        }
      }
      ...
      for (var i = 1; i <= 5; i++) {
        this.context.beginPath()
        this.context.moveTo(this.pWidth - 50, this.pHeight - 50 - i * yInterval)
        this.context.lineTo(this.pWidth - 50 + 5, this.pHeight - 50 - i * yInterval)
        this.context.strokeStyle = '#000000'
        this.context.stroke()
        this.context.closePath()
        ...
        this.context.fillText(yVal, this.pWidth - 50 + 10, this.pHeight - 50 - i * yInterval + 5)
      }
    },
    initHoverEvent() {
      var opts = this
      this.canvas_cover.addEventListener(
        ...
        function(e) {
          var arr = 0
          if (e.offsetX <= (opts.pWidth - 50) / 2) arr = opts.getPoint(opts.buyPoints, e.offsetX)
          ...
          if (arr != null) {
            ...
            opts.context_cover.beginPath()
            opts.context_cover.rect(rx, ry, 210, 80)
            opts.context_cover.fillStyle = '#f6f6f6'
            opts.context_cover.fill()
            opts.context_cover.closePath()

            /*填充文本*/
            opts.context_cover.beginPath()
            opts.context_cover.fillStyle = '#111111'
